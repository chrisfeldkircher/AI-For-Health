# HeAR audio preprocessing, vendored from Google-Health/hear
# (python/data_processing/audio_utils.py, Apache License 2.0, Copyright 2025 Google LLC).
# We vendor it so the extraction cell is self-contained (no repo clone / import shim).
#
# The functions _enclosing_power_of_two, _compute_stft, _ema, _pcen_function,
# _hertz_to_mel, _linear_to_mel_weight_matrix, _mel_pcen,
# _torch_resize_bilinear_tf_compat, preprocess_audio and
# resample_audio_and_convert_to_mono are COPIED VERBATIM from that file (upstream
# commit fetched 2026-07-06). They are the reference path.
#
# We ADD preprocess_windows(): a batched equivalent of preprocess_audio whose ONLY
# difference is that the [-1, 1] min/max normalisation is computed PER ROW instead
# of over the whole tensor. This matters because the upstream _mel_pcen normalises
# with a global torch.min/torch.max, so feeding a [W, 32000] batch would couple the
# W clips' scaling. Per-row normalisation makes each 2 s window's embedding
# batch-invariant and identical to calling the reference preprocess_audio on that
# window alone. test_matches_reference() proves preprocess_windows == preprocess_audio
# for the single-clip case (where per-row == global by construction).
#
# Upstream source: https://github.com/Google-Health/hear
"""HeAR mel-PCEN preprocessing (vendored, Apache-2.0)."""
...
def _torch_resize_bilinear_tf_compat(images, size):
  """PyTorch implementation of tf.image.resize (bilinear, no antialias)."""
...
def preprocess_audio(audio: torch.Tensor) -> torch.Tensor:
  """Preprocesses audio (REFERENCE PATH, verbatim from upstream).
...
def resample_audio_and_convert_to_mono(
  ...
  """Resamples an audio array to 16kHz and converts it to mono (verbatim)."""
...
def _mel_pcen_perrow(x: torch.Tensor) -> torch.Tensor:
  """Identical to _mel_pcen but the [-1, 1] scaling is PER ROW (dim=-1).

  For a single row this is bit-for-bit the same sequence of ops as _mel_pcen
  (global min/max == row min/max when there is one row); for a [W, 32000] batch
  it decouples the W windows so each is normalised on its own, matching what
  preprocess_audio would produce for each window in isolation.
  """
  x = x.float()
...
def preprocess_windows(windows: torch.Tensor) -> torch.Tensor:
  """Batched preprocess_audio with per-window normalisation.

  Args:
    windows: `[W, samples]` float tensor of 2 s windows (samples <= 32000; short
      windows are zero-padded on the right, exactly like preprocess_audio).

  Returns:
    `[W, 1, 192, 128]` mel-pcen tensor ready for the HeAR ViT.
  """
  if windows.ndim != 2:
    raise ValueError(f'windows must have rank 2, got rank {windows.ndim}')
  if windows.shape[1] < CLIP_SAMPLES:
    windows = F.pad(windows, (0, CLIP_SAMPLES - windows.shape[1]))
  elif windows.shape[1] > CLIP_SAMPLES:
    raise ValueError(f'windows must have <= {CLIP_SAMPLES} samples, '
                     f'got {windows.shape[1]}')
  spec = _mel_pcen_perrow(windows)
  spec = torch.unsqueeze(spec, dim=1)  # [W, 1, T, 128]
  return _torch_resize_bilinear_tf_compat(spec, size=(192, 128))


def test_matches_reference(device: str = 'cpu', seed: int = 0) -> float:
  """Proves preprocess_windows == preprocess_audio for single clips.

  Returns the max abs difference over a few random clips (should be ~0).
  """
  g = torch.Generator(device='cpu').manual_seed(seed)
  worst = 0.0
  for n in (32000, 32000, 20000, 5000):
    clip = torch.rand((1, n), generator=g, dtype=torch.float32).to(device)
    ref = preprocess_audio(clip)
    ours = preprocess_windows(clip)
