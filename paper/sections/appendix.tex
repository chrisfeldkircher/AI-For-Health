...
\paragraph{shadow\_seed=23 outlier note.} The lowest shadow split ($0.6443$, $\sim 2.6\sigma$ below shadow mean) is the most extreme of the 10 splits. With $\sigma_{\text{shadow}} \sim 0.017$ and $N{=}10$ shadow splits, the most extreme is expected to be $\sim 1.8\sigma$ from the mean by extreme-value statistics; observing $-2.6\sigma$ once is plausible but slightly outside the 90\% confidence interval. A targeted per-pseudo-speaker diagnostic (cell §4.13.1.1 in the project repository) checks whether shadow\_seed=23 has structurally harder cold cases (concentrated voice-quality outliers, specific cold-speaker clusters in the test fold) or just statistical fluctuation; the verdict feeds back into the partition-variance framing of Section~\ref{sec:disc} as either a single-outlier paper-stage caveat or a structural-difficulty paper-relevant finding.
