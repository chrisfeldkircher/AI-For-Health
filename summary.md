...
- **A5.5** — *Phase 1-2 DONE; Phase 3 FAILED; Phase 3.5 DONE → branch C → audio splicing DEAD; PIVOTED to Plan B (embedding mixup), Phase 2-equiv build queued.* Audio-level cross-speaker splicing was the original v1 design. Phase 1 (splice primitives) + smoke 77.3% DONE. Phase 2 (K=3 augmented pooled cache, 80.5% successful splices per variant, 10.9 min on GPU) DONE. **Phase 3 splice-detector audit FAILED at UAR 0.998** vs originally-specified gate ≤ 0.55. **Phase 3.5 self-splice control DONE** (`results/A5_5_phase3p5_selfsplice_control.json`): self-splice UAR 0.9900 ≈ cross-splice UAR 0.9981 (Δ -0.008 layer-averaged). **Cross-speaker mixing accounts for <1% of detectability; the splicing OPERATION itself accounts for ~99%.** No partner-selection refinement (same-session, same-speaker, RMS-matched) can fix this — the splicer is fundamentally artifact-laden (crossfade discontinuities + spectral boundary mismatches detectable across every WavLM layer L0–L24). **Audio-level splicing is dead on this corpus + this backbone.** **PIVOT to Plan B (embedding mixup, plan §4.8.5):** mix WavLM pooled stats per chunk (`α · anchor_pooled + (1-α) · partner_pooled`, α ∼ U(0.70, 0.85), partner = same Cold + different pseudo-speaker) instead of audio. Sidesteps every audio-level artifact (no crossfade, no boundary, no session mismatch — pure tensor blend). Pure CPU op on cached pooled stats: ~1-2 min for K=3 mixed variants on 8532 chunks. **Revised gate framing for ALL cross-speaker augmentations**: detector UAR is *descriptive* (augmentation-class baseline, expected non-zero by construction); the actual shortcut gate is **mix-bit-as-cold-predictor ≤ 0.52** (~0.5 expected by class-balanced augmentation design). **Methodological warning (paper-worthy, transferable):** "Audio-level cross-speaker splicing on URTIC WavLM representations is detectable at >99% UAR even within a single speaker; anyone doing audio-level cross-speaker augmentation on foundation-model representations should run a self-splice control before proceeding." Phase 2-equiv (mixup cache build) + audit + Phase 4 (training on mixed embeddings, warm-start from A2.5) queued. See [plan.md §4.8.4-4.8.6](plan.md) for full pivot documentation.
