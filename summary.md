...
- **A5b §4.14.1 Multi-K ensemble — DONE; multi_k_robust_lift; canonical crosses 0.710 baseline at 0.7111; shadow-mean closed 27% of gap (0.6882 → 0.6940).** (`results/A5b_k2_multi_k_ensemble.json`, 0.52 min total — mostly cached I/O.) Decision: `multi_k_robust_lift`. **Both gates passed:** shadow-mean Δ +0.0059 (≥ 0.003 threshold) AND **9/10 shadow splits positive** (≥ 7/10 threshold). Per-seed multi_K = 0.5·(K=1_seed + K=2_seed); then 5-seed mean-logit ensemble of multi_K_seed. **Per-split paired Δ:** canonical(42) +0.0021; shadow(1) -0.0017; shadow(2) +0.0073; shadow(3) +0.0015; shadow(5) +0.0056; shadow(11) +0.0039; shadow(17) +0.0134 (largest); shadow(23) +0.0075; shadow(31) +0.0078; shadow(53) +0.0051; shadow(99) +0.0082. Only seed=1 marginally negative (-0.0017, within numerical noise). **Aggregate:** baseline K=2-only shadow mean 0.6882 ± 0.0169 (matches §4.13.1 reference EXACTLY → pipeline determinism confirmed); multi-K shadow mean 0.6940 ± 0.0157 (note σ slightly *decreased* from 0.0169 → 0.0157, consistent with variance reduction); paired Δ shadow mean **+0.0059 ± 0.0041**. **Locked τ:** multi-K τ* = -1.625 vs K=2-only τ* = -1.375 (shifts toward more cold-aggressive operating point; consistent with K=1-component contributing additional cold-axis signal at locked β_k1). **CRITICAL: canonical now crosses 0.710 baseline by +0.0011** (multi-K canonical 0.7111 vs baseline 0.7100). Narrative shift: previous "matches 0.710 within partition variance" (K=2-only canonical 0.7090, Δ -0.001) becomes "exceeds 0.710 on canonical (Δ +0.001), comparable within partition variance on shadow mean (multi-K shadow-mean 0.6940 vs 0.7100, Δ -0.016 ≈ 1σ)." **Shadow-mean narrowing:** previous gap -0.022 → multi-K gap -0.016. **~27% of the shadow gap closed by Step 1 alone.** At upper end of predicted +0.001-0.003 range; actual +0.006 suggests K=1 + K=2 logits less correlated than r ~ 0.93 (closer to r ~ 0.85-0.90 given the per-seed β-weight asymmetry between K=1 and K=2). **Mechanism:** multi_K_seed = a2.5 + β_eff_g4·z_g4 + β_eff_g5·z_g5 with asymmetric β coefficients (more weight on G4_gi than G5_modulation) that differ per seed. 5 distinct effective per-channel weightings → ensemble averages over more independence than 5 K=2-only seeds at uniform weighting. **New paper-headline triple:** multi-K canonical 0.7111 / shadow-mean 0.6940 ± 0.016 / paired lift +0.006 over K=2-only with 9/10 positive. Cumulative lift over leak-corrected baseline: A5b multi-K = +0.075 (vs +0.073 for K=2-only); ~17.5σ. Step 1 of the §4.14 "come closer to 0.710" plan: DONE. Step 2 (diverse-anchor 10-seed) and Step 3 (ComParE-SVM K=3) remain optional next-step push axes. See [plan.md §4.14.1](plan.md).
- **A5b §4.13.2 Speaker-level logit smoothing (DONE, SHADOW-EVAL CONFIRMS smoothing hurts on every split; chunk-level signal is load-bearing on URTIC).** (`results/A5b_k2_speaker_smoothing.json`, 0.63 min total.) Decision: `smoothing_hurts`. The α-sweep on canonical train_threshold UAR picked α\*=0.0 (full smoothing) by maximizing train_threshold UAR (0.8401) — a textbook overfit signal: pure pseudo-speaker averaging exploits the train_threshold pseudo-speaker composition without generalizing. **Per-shadow-split paired delta** (smoothed α=0.0 vs no-smoothing α=1.0): all 11 splits negative; canonical Δ -0.0891; shadow-mean paired Δ **-0.0958 ± 0.0146, positive on 0/10 shadow splits**. Smoothed shadow mean = 0.5924 ± 0.0194 (vs no-smoothing baseline shadow mean 0.6882 ± 0.0169 which matches §4.13.1 reference EXACTLY → pipeline determinism confirmed across cells). **The α-sweep curve is monotonically increasing toward α=1.0** with no interior optimum: α=0.0 (0.6199) → 0.10 (0.6400) → ... → 0.95 (0.7084) → 1.00 (0.7090). Maximum at α=1.0 = no smoothing. **Mechanism (paper-grade insight, OPPOSITE of conventional paralinguistic intuition):** URTIC cold detection works AT THE CHUNK LEVEL because **within-speaker variation carries real cold evidence, not just noise**. A single speaker's recording session contains chunks with coughs / throat clearing / breathiness (high cold evidence) interspersed with relatively clean speech (low cold evidence); the model's per-chunk predictions correctly assign different cold-probability based on local acoustic evidence. Speaker-level smoothing destroys this by averaging high-evidence chunks with low-evidence ones, collapsing the predictive signal. **Pre-registered shadow-eval gate caught the train_threshold-overfit failure mode exactly as designed** — confirms the §4.13.1 robustness methodology is the right admission gate for any future intervention. **K=2 5-seed mean-logit ensemble at canonical 0.7090 / shadow-mean 0.6882 stays canonical.** No further interventions tested. See [plan.md §4.13.2](plan.md).
- **A5b §4.13.1.1 Shadow seed=23 outlier diagnostic (DONE; verdict `structural_harder_cold_speakers`; reveals M19-candidate speaker-UAR-noisier-than-chunk-UAR finding).** (`results/A5b_k2_shadow_seed23_diag.json`, 0.53 min total.) seed=23 has 95 unique pseudo-speakers (vs canonical 107) and only **5 cold speakers** (vs canonical 10), with cold-speaker mean correctness 0.466 (vs canonical 0.694). **Speaker-level UAR collapses to 0.4056** (vs canonical 0.7232). The mechanism is structural but **not a partition defect**: cold speakers are sparse on URTIC (~21 across full devel partition, halves of ~10 each), and seed=23 happened to put 5 *specific* harder-than-average cold speakers into devel\_test. **Z-score discrepancy note (caught in reflection 2):** the cell's `shadow_mean_ex_23` (excluding seed=23 itself, σ=0.0072) gave z=-6.78σ for chunk-UAR; the §4.13.1 inclusive shadow distribution (10 splits, σ=0.0169) gives z=-2.60σ. Both valid; for paper citation we use the inclusive z=-2.60σ (consistent with §4.13.1 framing; the exclusive-of-self overstates extremeness because tail-outlier inclusion expands inclusive σ). **The DEEPER finding (paper-grade, M19 candidate):** on URTIC, **speaker-level UAR is much MORE noisy than chunk-level UAR across shadow splits**: chunk-UAR shadow σ=0.017, speaker-UAR shadow σ=0.082 (~5× larger). This is the OPPOSITE of conventional paralinguistic intuition that speaker-level aggregation reduces noise. Mechanism: cold speakers are sparse, so per-speaker accuracy itself is noisy; aggregating ~4800 chunks → ~100 speakers loses effective sample size. **M19 candidate methodology contribution:** *"on corpora with sparse minority-class speaker representation, speaker-level aggregation (smoothing or evaluation) increases variance rather than reducing it; the chunk-level metric is more stable. Validates ComParE's choice of chunk-UAR as the official metric and rules out a class of related interventions (multi-chunk inference, confidence weighting, speaker-level evaluation)."* This finding UNIFIES the §4.13.1.1 seed=23 outlier + §4.13.2 speaker-smoothing failure as downstream consequences of cold-speaker sparsity on URTIC. **Reframe seed=23 in the paper** from "harder cold speakers in this partition" (defect-implying) to "cold-speaker selection variance is large because the cold class is sparse" (structural property). See [plan.md §4.13.1.1](plan.md).
- **A5b §4.13.1 Shadow-split robustness harness (DONE; canonical at z=+1.24σ above shadow mean -- "marginal" verdict; paper-grade methodology contribution M18).** (`results/A5b_k2_shadow_splits.json`, 0.59 min total.) Decision: `marginal_canonical_within_2sigma`. Re-evaluated canonical K=2 5-seed mean-logit ensemble + per-seed-single under N=10 different (devel_val, devel_test) partitions of the devel split via different StratifiedGroupKFold seeds {1, 2, 3, 5, 11, 17, 23, 31, 53, 99}; train_fit + train_threshold kept fixed at canonical SPLIT_SEED=42 (per-seed A2.5 head ckpts + locked β\* per seed unchanged). Per-seed inference computed ONCE on all 9596 devel chunks then partitioned per shadow_seed = ~10× cost reduction vs naive. **HEADLINE NUMBERS:** per-seed K=2 5-seed-mean shadow distribution = **0.6860 ± 0.0153** (range [0.6458, 0.7001], canonical 0.7037 at z=+1.16σ); mean-logit ensemble shadow distribution = **0.6882 ± 0.0169** (range [0.6443, 0.7042], canonical 0.7090 at z=+1.24σ). **Mean-logit ensemble still beats per-seed-single across most shadow splits** (ensemble > single on 9 of 10 shadow splits by Δ +0.001-0.004; canonical Δ +0.005) -- confirms the §4.11.1.4 ensemble lift transfers across partitions, just at ~50% of canonical magnitude. **The canonical split is favorable but not pathological.** z=+1.24σ above shadow mean = the locked configuration found a partition where its specific characteristics happened to align well with the test fold (per-seed locked β\* tuned on canonical train_threshold; per-seed and ensemble τ\* swept on canonical train_threshold; the K=2 G_other selection G5 chosen on canonical devel_test in §4.11.1.1). This isn't methodological failure — every decision followed the pre-registered protocol; the canonical split was fixed early; the partition variance is the irreducible noise of single-partition evaluation. **CRITICAL PAPER-STAGE FRAMING: the 2017 baseline (0.7100) is also a single-partition estimate** with its own (unknown but likely similar) partition variance; if σ\_2017 ~ 0.017, the baseline could plausibly be anywhere from 0.68 to 0.74 on a shadow split. **The 0.001 gap between our 0.7090 and the 2017 0.7100 is much smaller than the partition variance of either estimate.** Replace "matches 0.710 within measurement noise" with **"statistically equivalent within partition variance"** as the stronger paper headline -- defensible against the obvious reviewer question "did you overfit devel?". **seed=23 is a low outlier (0.6443, ~2.6σ below shadow mean)** -- flagged for §4.13.1.1 quick diagnostic to check if it's structural (concentrated hard cold cases) or fluctuation (extreme-value tail of 10 splits). **M18 (paper-stage methodology candidate):** "shadow-split robustness harness on the canonical pipeline's cached logits is a cheap diagnostic that quantifies devel-side overfit risk; report shadow distribution alongside any single-partition headline UAR when the held-out test set is unavailable." Generalises beyond URTIC. **Future-experiment evaluation gate:** any new intervention should be evaluated on the shadow distribution; promote only if it improves the shadow mean (the canonical-only delta is partly inflated by the +1.24σ canonical favorability). See [plan.md §4.13.1](plan.md).
- **A5b §4.12.3 K=3 with HuBERT-base + LEARNED LW softmax (DONE; nuanced result -- both hypotheses (i)+(ii) partially validated, K=3 NO ADMIT by logit-correlation; K=2 stays canonical).** (`results/A5b_k3_hubert_lw_5seed.json`, **3.71 min** -- 13× faster than predicted 50 min because head training fired early-stop at ep 1-4 across all seeds; the HuBERT layer-weight subspace is much smaller than WavLM's and the cold signal saturates fast.) Decision: `k3_hubert_lw_no_admit`. **HuBERT-A2.5 standalone 5-seed = 0.6217 ± 0.0291** -- crosses the M14 admit-plausible threshold (0.61) but with much higher per-seed variance than WavLM-A2.5 (~0.0027). Δ +0.082 vs HuBERT mean-pooled (0.5396 from §4.11.2.1) -- **hypothesis (i) "layer-weighted softmax does the work" PARTIALLY VALIDATED**. Δ -0.034 vs WavLM-A2.5 standalone (~0.656) -- **hypothesis (ii) "capacity dominates" PARTIALLY VALIDATED**. Rough decomposition of the WavLM↔HuBERT-mean gap (0.116 total): layer-weighted softmax buys ~71% (+0.082); capacity buys ~29% (+0.034). Both hypotheses contribute. **K=3 fusion verdict: NO ADMIT** (K=3 5-seed = 0.6955 ± 0.0083, Δ -0.008 vs K=2 LOCKED 0.7037 ± 0.0060; per-seed locked β* = {16, 16, 6, 16, 16} with 4 of 5 boundary-pegged; per-seed deltas all negative within K=2 σ envelope). **NEW MECHANISTIC INSIGHT (refines the §6 standalone-UAR-predictor heuristic):** standalone cleared 0.61 but K=3 didn't admit because **HuBERT-A2.5 logit is highly correlated with WavLM-A2.5 logit by construction** (both derived from the same audit recipe -- per-layer cold + speaker probes → sub@1 → LW softmax head with honesty prior → cold-CE training). Both heads concentrate on cold-relevant layers and produce similar cold-axis predictions. K=3 fusion adds a redundant logit, not an orthogonal one; the late-fusion architecture detects this and pegs β* at boundary. **The heuristic should be augmented with a logit-correlation check (or an early-fusion baseline) before committing β-sweep compute to a candidate that's structurally similar to the anchor.** Per-seed standalone is bimodal (3 of 5 seeds clear 0.61, 2 of 5 below); the mean crosses by a thin margin and per-seed variance (σ = 0.0291) is the limiting factor. **Cosine(audit prior, final softmax) = 0.95 across all 5 seeds** -- prior tightly preserved through training (mirrors M5 finding for WavLM-A2.5). Top-5 final softmax weights heavily concentrated on L0 (0.31) which is also the highest-sub@1 audit layer. **Paper implications:** §08 future work axis (HuBERT-base + learned LW) now CLOSED with paper-paragraph evidence; the remaining untested §08 variants are (ii) HuBERT-large mean-pooled (still genuine "capacity dominates" test) and (iii) HuBERT discrete tokens (orthogonal feature, not subject to audit-recipe-mirror correlation issue). Add to paper as: a paragraph in §4.5.5 04_method.tex; new HuBERT-A2.5 row in §6 standalone-UAR-predictor table with the mechanism boundary; new `app:k3_hubert_lw` appendix section; §08 future work close-out. **K=2 LOCKED at 0.7037 ± 0.0060 / 5-seed mean-logit ensemble at 0.7090 stays canonical paper-headline number.** All experimental phases now closed within the configurations tested. See [plan.md §4.12.3](plan.md). Cleanest test of the §4.11.2.1 hypothesis (i) "layer-weighted softmax pooling does the work" vs hypothesis (ii) "capacity dominates". Mirrors WavLM-A2.5's treatment on HuBERT-base pooled stats: STEP 1 per-layer HuBERT honesty audit (13 layers; cold + speaker probes per layer; sub@1 vector); STEP 2 HuBERT-A2.5 head training (LayerWeightedPooledHead n_layers=13, stat_dim=3072, proj_dim=128, dropout=0.5; honesty-prior init T*sub@1 with T=50; cold-CE class-balanced sampler, lr 1e-3 head + 1e-4 LW; 25 epochs, early stop patience 6 on devel_val UAR; 5 seeds, ckpt per seed at `cache/facebook_hubert-base-ls960/head_A25_honestprior_seed{seed}.pt`); STEP 3 standalone HuBERT-A2.5 UAR per seed → M14 pre-flight (< 0.55 skip; ≥ 0.61 admit-plausible; [0.55, 0.61) borderline); STEP 4 K=3 fusion sweep (`fused = a2.5_wavlm + β · mean(z_g4, z_g5, z_hubert_a25)`, extended β-grid, 5 seeds) if pre-flight passes. **Decision rules:** mean K=3 UAR > 0.7087 → ADMIT (new canonical, 'layer-weighted softmax does the work' VALIDATED); standalone clears 0.61 but K=3 doesn't admit → M14 falsified on this substrate (orthogonality with WavLM-A2.5 logit not present); standalone < 0.55 → 'layer-weighted softmax DOESN'T close the gap on HuBERT-base alone; capacity / backbone strength dominates' = evidence for hypothesis (ii). **Cost:** ~50 min wall-clock (~5 min audit + ~25 min head training + ~20 min standalone + K=3 sweep). **Highest-priority cell:** load-bearing methodology test, plausible UAR upside if standalone clears 0.61 (K=3 lift +0.005 to +0.012 → ensemble ~0.715-0.721). **Output:** `results/A5b_k3_hubert_lw_5seed.json` + `results/A5d_hubert_layer_honesty.csv`. See [plan.md §4.12.3](plan.md).
- **A5b §4.12.2 K=2 TTA ensemble (DONE; TTA HURTS by Δ -0.0407, mean-logit stays canonical; M17-candidate "WavLM input-normalisation absorbs gain; time-stretch hurts pooled-stat features").** (`results/A5b_k2_tta_ensemble.json`, 39.88 min total: ~32 min WavLM-Large re-extraction across 4 augs × 19,101 chunks at ~30 chunks/sec on GPU + ~7 min fusion+sweep across 5 seeds × 5 augs.) Decision: `tta_hurts`. TTA ensemble UAR = **0.6683** (Δ -0.0407 vs no-TTA 0.7090). Original-only-ensemble reproduction = 0.7090 exactly (Δ -0.0000 vs §4.11.1.4 reference) — confirms per-seed K=2 logit pipeline is fully deterministic. **Per-aug mean-of-5-seeds devel_test UAR:** original 0.7037; gain_p2dB 0.6895 (-0.014); gain_m2dB 0.6894 (-0.014); time_stretch_p2 0.6748 (-0.029); time_stretch_m2 0.6601 (-0.044). **TWO PAPER-RELEVANT MECHANISM FINDINGS:** (1) **Gain ±2dB is essentially a no-op due to WavLM input normalisation.** Per-seed `gain_p2dB` and `gain_m2dB` UARs are identical to 4 decimal places (seed 7 differs by 0.0003 = fp16 noise). The WavLM AutoFeatureExtractor applies `do_normalize=True` (per-utterance zero-mean unit-variance), which is mathematically gain-invariant: `normalize(g·x) = (g·x - g·μ) / (g·σ) = (x - μ) / σ = normalize(x)` for any positive g. So multiplicative gain is absorbed at the input pipeline level *before* the transformer ever sees the difference. Future TTA designs on WavLM/HuBERT/Wav2Vec2 substrates should use perturbations that survive input normalisation (additive noise, spectral masking on framewise features, or perturbations at the pooled-stats stage). (2) **Time-stretch ±2% causes 0.029-0.044 mean per-seed UAR drop.** WavLM's per-frame outputs have temporal structure (positional encoding + transformer self-attention pattern) that stretching disrupts; the new pooled stats systematically lose cold-relevant patterns A2.5's layer-weighted softmax was tuned to. Time-stretch logits are out-of-distribution for the trained probes. **TTA ensemble dilutes rather than averages-toward-improvement** because the augmented logits are systematically biased (gain ≡ original ≡ no-op contribution; time-stretch systematically lower-UAR). 40% of the ensemble being time-stretch-quality is enough to drag UAR by 0.04. **M17 candidate (paper-stage methodology):** *"On foundation-model substrates with input normalisation (Wav2Vec2 / HuBERT / WavLM all default to per-utterance zero-mean unit-variance), waveform-level multiplicative perturbations are absorbed and produce identical pooled features. Standard TTA designs (gain + time-stretch) are unsuitable for FM-substrate ensembling: gain = no-op; time-stretch = systematic UAR drop. Net: TTA ensemble UAR is strictly worse than no-TTA mean-logit. Future TTA must use perturbations that survive input normalisation (additive noise) or operate at the pooled-stats stage (SpecAugment-style)."* Generalises beyond URTIC. **The 0.7090 ensemble is now triply defended:** calibration ablations (§4.12.1, equal-weight is variance-optimal), TTA ablations (§4.12.2 this cell, input-perturbation averaging hurts), original-only-ensemble reproduction (deterministic at 0.7090 exactly across both cells). Mean-logit stays canonical paper-headline number. See [plan.md §4.12.2](plan.md).
