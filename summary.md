...
- **A5b §4.12.1 K=2 ensemble calibration + stacked weighting (Tier-2; cell appended, QUEUED for user-run).** Three weighting strategies on the K=2 5-seed per-seed fused logits + one isotonic ablation: (1) MEAN-LOGIT baseline (existing 0.7090, equal weights w_i=0.2); (2) LR-STACKED (StandardScaler + LogisticRegression on per-seed K=2 logits → cold; 5 learned weights + bias collapse 5-d → 1-d calibrated probability); (3) UAR-GRID-SEARCH (coarse grid over (w_1..w_5) ∈ {0.0, 0.1, ..., 0.5} normalised to sum-1 → 7776 combos; pick max-UAR-on-train_threshold); (4) ISOTONIC ABLATION on the best of (1)-(3) — M15-candidate "monotonic calibration cannot improve UAR-at-optimal-tau" test (mathematically guaranteed null since UAR-at-swept-tau is invariant under monotonic transforms; non-zero lift would indicate measurement artefact). **Why M15 matters:** the existing tau-sweep already does whatever monotonic calibration would do. Confirmation in this cell would let us add M15 to the methodology table as a tight bound on calibration's reach for UAR-at-swept-threshold. **Expected lift:** +0.000 to +0.005 over 0.7090 if some seeds add noise (LR shrinks them down) or non-monotonic seed-quality (grid search captures it). **Decision rules:** ≥ 0.710 → "crosses baseline"; > 0.7090 + 0.002 → "modest help"; |Δ| ≤ 0.002 → "neutral"; < 0.7090 - 0.002 → "hurts". **Cost:** ~5 min wall-clock (no GPU; numpy + sklearn only on cached fused logits). **Output:** `results/A5b_k2_ensemble_calibrated.json`. **Cheapest of the three Tier-2 cells; run first.** See [plan.md §4.12.1](plan.md).
