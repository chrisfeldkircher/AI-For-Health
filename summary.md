...
- **A2.5** — *NEW BASELINE (mechanism revised; canonical pending lr × init grid).* A2 + honesty-prior layer-weight init (logits = T·sub@1 from A5d) → **UAR 0.6564 ± 0.0038** on grouped splits at default lr (3 seeds). Δ +0.020 vs A2_grouped (4.7σ); MLP probe flat (0.0501 → 0.0501); LR probe down (0.0760 → 0.0725). Pareto improvement on every dimension. **Mechanism revision:** the original "flat landscape" claim (cos(init, final) = 0.9998 at default lr) is contradicted by the lr stress test ([results/A2_grouped_honestprior_lr_stress.json](results/A2_grouped_honestprior_lr_stress.json)) — at lr×100, cos drops to 0.21 with substantial layer-weight movement. There IS gradient signal; the standard 0.1× lr factor + 25-epoch budget isn't enough to express it. The corrected paper claim: *"data-derived layer-weight init produces +0.020 UAR over uniform init in the standard low-lr regime, where the optimizer is constrained from moving regardless of starting point. Practically useful within standard optimization settings rather than fundamentally necessary across all settings."* Single-seed lr×10 hint: UAR 0.6599 with prior init at lr×10 (suggests prior + medium-lr might dominate); 3-seed replicate + uniform-init control pending in §4.6.2. Canonical A2.5 lock deferred until lr × init grid lands. See [plan.md §4.6](plan.md) for full mechanism documentation.
- **A5b on A2.5** — *β=1.00 forced FAIL → β-sweep PASS at boundary, follow-up pending.* The original K=1 lock used β=1.00 (implicitly conditioned on uniform-A2 anchor). On A2.5 the protocol-strict β=1.00 run gave Δ -0.022 with τ pegged at search edge -3.95 (calibration failure, not signal absence — see [results/A5b_grouped_honestprior.json](results/A5b_grouped_honestprior.json)). β-sweep over {0, 0.05, 0.1, 0.25, 0.5, 0.75, 1.0, 1.5, 2.0} jointly with τ ([results/A5b_grouped_honestprior_betasweep.json](results/A5b_grouped_honestprior_betasweep.json)) recovered K=1 fused UAR 0.6755 ± 0.0208 at β*=2.0 (all 3 seeds at the upper search boundary), Δ +0.019 ± 0.018 vs A2.5_argmax (PASS gate, but ~0.7σ above gate threshold; per-seed: 1/3 neutral, 2/3 lift). β=0 sanity passes cleanly. **Caveat: β* pegged at search boundary** — same diagnostic that flagged K-sweep pathology. Extended β grid {2.5, 3, 4, 6, 8, 12, 16} + G4-alone baseline pending in §4.7.2. Two-stackable-contributions paper framing tentatively supported.
...
results/
  ...
  A5b_grouped_honestprior_betasweep.json  A5b β-sweep on A2.5 anchor (β grid {0..2.0}, PASS at β*=2.0 boundary, follow-up pending)
