...
- **A2.5** — *NEW BASELINE.* A2 + honesty-prior layer-weight init (logits = T·sub@1 from A5d) → **UAR 0.6564 ± 0.0038** on grouped splits. Δ +0.020 vs A2_grouped (4.7σ); MLP probe flat (0.0501 → 0.0501); LR probe down (0.0760 → 0.0725). Pareto improvement on every dimension. Optimizer didn't move the prior at all (cos(init,final) = 0.9998) — settles "why uniform-init A2 collapses to uniform" as flat loss landscape, not absence of differential signal. Becomes the new canonical A2 for downstream rungs. See [results/A2_grouped_honestprior.json](results/A2_grouped_honestprior.json) and [plan.md §4.6](plan.md).
...
results/
  ...
  A2_grouped_honestprior.json  A2.5: A2 + honesty-prior layer-weight init from A5d sub@1 (NEW canonical baseline)
