...
- **A5.5** — *Plan B Phase 2 + audit + Phase 4 DONE. Phase 4 PARTIAL PASS (UAR floor PASS, speaker probe drop FAIL on strict 3-D gate at α ∈ [0.70, 0.85]); α ∈ [0.5, 0.7] sweep queued.* History: Phase 1 (splice primitives) + smoke 77.3% DONE. Phase 2 (K=3 audio-spliced cache, 80.5% successful, 10.9 min GPU) DONE. **Phase 3 splice-detector audit FAILED at UAR 0.998**. **Phase 3.5 self-splice control DONE → branch C (splicer broken)**: self-splice UAR 0.9900 ≈ cross-splice 0.9981 (Δ -0.008); cross-speaker mixing accounts for <1% of detectability, splicing operation itself accounts for ~99%. **Audio-level splicing dead.** **Plan B (embedding mixup) DONE.** **Phase 2-equiv mixup cache** (`results/A5_5_planB_phase2_mixup.json`): 100% successful mixes (no fallback); α ∼ U(0.70, 0.85) realised mean 0.775 ± 0.043; partner-class balance 10.1% cold ≈ corpus 9.5% (symmetric); 1.13 min CPU. **Plan B audit** (`results/A5_5_planB_audit.json`): both gates PASS — (A) detector UAR layer-averaged 0.5034 (essentially chance, Δ -0.4947 vs Phase 3 audio splicing 0.998); (B) mix-bit-as-cold-predictor 0.5000 (trivial PASS by class-balanced construction, Δ cold rate mixed-vs-orig = 0.0000). **Plan B Phase 4 training** (`results/A5_5_planB_phase4_mixup.json`, 3 seeds, warm-start from A2.5): UAR 0.6624 ± 0.0031 vs A2.5 0.6564 (Δ +0.0060, ~1.6σ); recC 0.474 vs 0.43 (+0.04 — more cold-balanced); MLP probe 0.0506 ± 0.0019 vs A2.5 0.0501 (Δ +0.0005); LR probe 0.0759 ± 0.0030 vs A2.5 0.0725 (Δ +0.0034 — slightly UP); cos(init_A2.5, final) = 0.9999/1.0000/0.9997 across seeds (optimizer didn't move from A2.5's converged state — consistent with M5; +0.006 UAR is MLP/classifier-level adjustment to augmented training distribution, NOT layer-weight refinement). **3-D gate verdict: gate 1 (UAR floor) PASS, gates 2a/2b (speaker probe drop ≥ 1σ) FAIL, gate 3 (mix-bit shortcut) PASS — overall FAIL on strict 3-D gate.** Diagnosis: α=0.775 mean (22.5% partner contribution) too gentle to push the model toward speaker-invariance — augmentation activated weak regularization (recall pattern shifted to more cold-balanced) but didn't activate de-confounding mechanism. **α ∈ [0.5, 0.7] sweep queued (plan §4.8.8)** to test whether 35-50% partner contribution activates the de-confounding mechanism. Three branches: (a) UAR holds + probe drops → strict PASS; (b) UAR holds + probe stays → confirmed limitation, lock as "modest UAR / null on probe", move to A6; (c) UAR drops + probe drops → Pareto trade-off documented. ~30 min compute. See [plan.md §4.8.4-4.8.8](plan.md) for full pivot + Plan B documentation.
...
results/
  ...
  A5_5_planB_phase4_mixup.json    A5.5 Plan B Phase 4 training: UAR 0.6624 (Δ +0.006 vs A2.5); MLP/LR probes ~unchanged → PARTIAL PASS (3-D gate FAIL on probe drop)
