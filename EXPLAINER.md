...
**(M8) Cross-speaker audio splicing on URTIC's WavLM representation is acoustically detectable; the original splice-detector gate (≤ 0.55) was miscalibrated for this augmentation type** ([plan §4.8.3 / §4.8.4](plan.md), [results/A5_5_phase3_splice_detector.json](results/A5_5_phase3_splice_detector.json)). Phase 3 of A5.5 found that a binary LR probe distinguishes original vs spliced WavLM pooled features at **UAR 0.998** — perfect detection across every layer (L0/4/7/12/18/24) and every k variant. The originally-specified gate (≤ 0.55, plan §6) was likely calibrated for *perceptual-equivalence* augmentations (time-warp, noise injection, mild EQ) where the augmented chunk should be acoustically indistinguishable from the original. Cross-speaker splicing is a different augmentation class — pooled stats are mean+std+skew+kurt over time per WavLM layer; replacing 25-30% of a chunk with audio from a *different speaker* shifts those per-layer time-means by 25-30% toward partner's per-layer mean (the same property ECAPA exploits for speaker recognition). The augmentation is *supposed* to produce different pooled stats — that's the mechanism by which it breaks the (chunk → anchor speaker) link. **The actual shortcut concern (splice presence correlated with cold) is structurally mitigated by symmetric-across-classes splicing** — every chunk regardless of cold gets K=3 augmented variants (partner-pool: same Cold + different pseudo-speaker), so the model can detect "this is spliced" with UAR 0.998 *and still not be able to use it to predict cold*. The splice-bit-as-cold-predictor gives UAR ≈ 0.5 by construction. **Phase 3.5 self-splice control** (queued; ~3 min) — splice anchors with same-pseudo-speaker partners (no cross-speaker mixing) and re-run the same detector — diagnoses whether the 0.998 UAR is from cross-speaker mixing (the desired augmentation) or from splicer technique itself (real artifact). Self-splice UAR < 0.6 → splicer clean, gate redefinition documented; 0.6-0.85 → splicer fixes; > 0.85 → pivot to **embedding mixup** (mix pooled vectors instead of audio — sidesteps every audio-level artifact, well-precedented for cross-speaker emotion/accent classification). **For the paper, this is a methodological refinement worth a sentence**: the splice-detector audit's gate target needs to be augmentation-class-aware — different augmentation families have different baseline detectability, and cross-speaker mixing has a non-zero baseline by construction.
...
- **A5.5 IN PROGRESS, Phase 3 FAIL diagnostic in flight.** Phase 1 (splice primitives, smoke 77.3%) DONE; Phase 2 (K=3 augmented pooled cache, 80.5% success, 10.9 min) DONE; **Phase 3 splice-detector audit FAILED at UAR 0.998 vs originally-specified gate ≤ 0.55** — diagnosis: gate likely miscalibrated for cross-speaker splicing (pooled-stats inherently shift toward partner's speaker characteristics; cf. M8). Phase 3.5 self-splice control queued (~3 min) — distinguishes "splicer technique broken" from "cross-speaker mixing is the loud signal." Three branches: < 0.6 → gate redefinition + proceed to Phase 4; 0.6-0.85 → splicer fixes; > 0.85 → pivot to embedding mixup (Plan B). Phase 4 (head training + 3-D acceptance gate, on A2.5 anchor warm start, per-epoch sampling from {original, k=0/1/2}, anchor pseudo-speaker labels for augmented chunks, devel unaugmented) pending Phase 3.5 verdict.
