...
results/
  ...
  A5b_grouped.json        A5b K=1 lock + locked-K speaker probes on grouped splits (leak-corrected, paper headline)
...
- **A2.5 honesty-prior init: A5d converted from descriptive to architectural prior.** Side-experiment motivated by the reflection's question — "is A5d's per-layer signal architecturally exploitable?" Initialised A2's softmax layer-weights with `T_INV * sub@1` (T_INV=50) instead of zeros. Result: **+0.020 UAR (4.7σ) free**, MLP probe flat (0.0501 → 0.0501), LR probe slightly down (0.0760 → 0.0725). Pareto improvement on every measured dimension. The mechanistic finding: `cos(init_softmax, final_softmax) = 0.9998` across all seeds — the optimizer literally did not move the prior. This settles "why uniform-init A2 collapses to uniform" — not because uniform was the optimum, but because the loss landscape has no gradient pressure to specialise (the head's MLP can solve cold prediction regardless of layer mix). Generalises beyond URTIC: for foundation-model layer weighting in low-data settings, "let the optimizer figure it out" can fail silently. A2.5 becomes the new canonical baseline for downstream rungs (A5.5, A6, A7); A5b K=1 will be re-evaluated on this anchor before its verdict is locked.
...
- **A5b on A2.5 anchor** — does K=1 fusion still add marginal lift after the honesty-prior baseline? (Pending re-run, ~10 min.)
- **5-seed re-run on A2.5** — current σ_argmax = 0.0038 at N=3; the val-test gap widening (-0.013 → -0.020) may need 5 seeds to settle.
- **A5.5 cross-speaker splicing on A2.5 anchor** — three-dimensional gate (UAR floor, speaker probe drop ≥ 1σ, splice-detector ≤ 0.55 UAR). In progress; Phase 1 (splice primitives) committed.
- **A6 contrastive pretraining + A7 MDD/DANN adversary** — main de-confounding rungs; not yet built.
- **Late-layers-only A2 baseline** (restrict to L18-24) — possible follow-up to A2.5 if a cleaner cold-vs-speaker tradeoff is wanted as an A6 substrate.
