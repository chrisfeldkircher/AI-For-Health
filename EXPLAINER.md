...
results/
  ...
  A2_grouped_honestprior.json  A2.5: A2 + honesty-prior layer-weight init at default lr (canonical baseline at default lr)
  A2_grouped_honestprior_lr_stress.json  A2.5 layer-weight lr × {0.1, 1, 10, 100} stress test (single seed)
  ...
  A5b_grouped_honestprior_betasweep.json  A5b β-sweep on A2.5 anchor (PASS at β*=2.0, boundary-pegged; pending extension)
...
- **A5b β-sweep on A2.5: PASS recovered at β*=2.0 (boundary-pegged, caveated).** β grid {0, 0.05, 0.1, 0.25, 0.5, 0.75, 1.0, 1.5, 2.0} swept jointly with τ on train_threshold (`results/A5b_grouped_honestprior_betasweep.json`). All 3 seeds locked β*=2.0 (the upper edge of the grid); K=1 fused UAR 0.6755 ± 0.0208, Δ +0.019 ± 0.018 vs A2.5_argmax (PASS the +0.007 gate by mean, ~0.7σ above gate threshold). β=0 sanity passes cleanly (reproduces A2.5_τ exactly). Per-β trend non-monotonic and bumpy — middle β (0.5-1.5) creates unfavorable τ-operating-point interference; low and high β both lift. Recall pattern: at β*=2.0, recC ≈ 0.82, recNC ≈ 0.53 (heavily cold-biased) vs A2.5's balanced 0.45/0.86 — the high-β fusion is qualitatively different from "stack G4_gi onto A2.5." **Caveat: β* at search boundary** (same diagnostic that flagged K-sweep + β=1.00 fail). G4-alone reference from A5a says G4_gi standalone UAR ~0.63 < A2.5 0.6564, so β→∞ does NOT dominate, but exact plateau is unknown without extending the grid. Verdict tentatively PASS for two-stackable-contributions framing; final lock pending extended β grid + G4-alone baseline + A2.5 lr × init grid.
...
**(A1) A2.5 — honesty-prior layer-weight init** ([plan §4.6](plan.md), [results/A2_grouped_honestprior.json](results/A2_grouped_honestprior.json)). Initialise WavLM layer-weight softmax logits = T·sub@1 from the per-layer audit (M3). Result on grouped splits at default lr: +0.020 UAR (4.7σ) over uniform-init A2_grouped, MLP probe flat, LR probe slightly down. Pareto improvement on every measured dimension. **Mechanism (revised by M5):** at default optimization settings (`layer_weights lr = base_lr × 0.1 = 1e-4`, the s3prl convention, with 25-epoch / patience-6 horizon), the layer-weight subspace shows weak gradient signal that fails to drive specialisation from uniform init within the standard horizon. The honesty prior provides a useful starting point that the optimizer accepts (cos(init, final) = 0.9998 at default lr). At higher lr the optimizer *does* discover non-uniform configurations from any starting point — different layer mixes, similar UAR (lr×10 single-seed = 0.6599) — but this requires hyperparameter changes that the standard recipe doesn't make. **Corrected paper claim:** *"data-derived layer-weight init produces +0.020 UAR over uniform init in the standard low-lr regime, where the optimizer is constrained from moving regardless of starting point. Practically useful within standard optimization settings rather than fundamentally necessary across all settings."* A2.5 is the **canonical baseline at default lr**; the canonical-A2.5 lock pending the lr × init grid (§4.6.2) to confirm whether prior + lr×10 dominates.
...
**Immediate (queued as control cells):**

- **A2 lr × init grid** — (uniform init, honesty-prior init) × (lr×3, lr×5, lr×10), 3 seeds each. Settles whether the prior contribution survives at higher lr (currently only lr×0.1 is multi-seed). If `uniform + lr×10 ≈ honesty + lr×10` reaches ~0.66, the (A1) prior contribution shrinks to "useful at standard lr only." If `uniform + lr×10` stays at ~0.64, the prior is genuinely informative across lr regimes and (A1)'s claim strengthens. ~36 min.
- **A5b extended β-sweep + G4-alone baseline** — extend the β grid to {2.5, 3.0, 4.0, 6.0, 8.0, 12.0, 16.0} to find where the per-β UAR plateaus (resolves the boundary-pegged β*=2.0). Add G4_gi-alone evaluation on devel_test (β→∞ limit) as a direct verification that G4 alone doesn't dominate (A5a says ~0.63 standalone; need confirmation on the same protocol). Add robust-β diagnostic (smallest β within 1σ of per-β UAR peak). ~5 min.

**Tracked (to settle before paper submission):**

- **5-seed re-run on the locked canonical A2.5** — current σ_argmax = 0.0038 at N=3; the val-test gap widening (-0.013 → -0.020 → -0.026 across lr factors) may need 5 seeds to settle. Both UAR and val-test gap stability should drive the canonical-anchor decision (the canonical A2.5 is the (init, lr) combination that jointly maximises UAR mean and minimises val-test gap drift).
- **A5.5 cross-speaker splicing on A2.5-locked anchor** — three-dimensional gate (UAR floor, speaker probe drop ≥ 1σ, splice-detector ≤ 0.55 UAR). In progress; Phase 1 (splice primitives) committed; Phase 2 (augmented pooled cache build) pending the canonical A2.5 lock.
- **A6 contrastive pretraining + A7 MDD/DANN adversary** — main de-confounding rungs; not yet built.

**Optional follow-ups:**

- **Late-layers-only A2 baseline** (restrict to L18-24) — possible follow-up to A2.5 if a cleaner cold-vs-speaker tradeoff is wanted as an A6 substrate. The lr stress test's "early layers preferred at higher lr" finding makes this less urgent (the optimizer already prefers early layers, not late ones, when given freedom to specialise).
- **β-sweep on uniform-A2 anchor as a control** — does the original A5b (A2-a) PASS still hold under a fine β sweep on the uniform anchor? If β=1.00 was *already* the right hyperparameter under uniform-A2, no new audit needed; if β-sweep on uniform-A2 also lifts, then the protocol's "fixed β" choice was always overly constrained.
