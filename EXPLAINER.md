...
- **A5.5 PIVOTED to Plan B (embedding mixup); audio splicing dead per Phase 3.5 verdict.** Phase 1 (splice primitives, smoke 77.3%) DONE; Phase 2 (K=3 augmented audio-spliced cache, 80.5% success, 10.9 min) DONE; Phase 3 splice-detector audit FAILED at UAR 0.998 vs originally-specified gate ≤ 0.55; **Phase 3.5 self-splice control DONE: branch C (splicer broken)** — self-splice UAR 0.9900 ≈ cross-splice UAR 0.9981 (Δ -0.008), cross-speaker mixing accounts for <1% of detectability. Audio-level splicing dead on this corpus + this backbone. Pivot to embedding mixup: mix WavLM pooled stats per chunk (`α · anchor + (1-α) · partner`, α ∼ U(0.70, 0.85)) instead of audio. Pure CPU op on cached pooled stats. Plan B Phase 2-equiv build (~1-2 min) + audit + Phase 4 training (warm-start from A2.5, per-epoch sampling from {original, mix_0/1/2}, devel unaugmented) queued. **Revised gate framing for ALL cross-speaker augmentations:** detector UAR is descriptive (augmentation-class baseline); the actual shortcut gate is mix-bit-as-cold-predictor ≤ 0.52 (~0.5 expected by class-balanced design). See (M8) for the full transferable methodological finding + paper framing.
