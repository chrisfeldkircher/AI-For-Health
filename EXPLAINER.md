...
results/
  ...
  A5_5_planB_phase4_mixup.json   A5.5 Plan B Phase 4: head training warm-start from A2.5; UAR Δ +0.006, probe ~unchanged → PARTIAL PASS (3-D gate FAIL on probe drop)
...
- **A5.5 PARTIAL PASS at α ∈ [0.70, 0.85]; α-sweep follow-up queued.** Audio-splicing v1 killed by Phase 3.5 (M8 self-splice diagnostic). Pivoted to embedding mixup; Plan B Phase 2 + audit BOTH PASS (detector UAR 0.503 essentially chance, Δ -0.4947 vs Phase 3 audio splicing 0.998; mix-bit-as-cold trivial PASS at 0.500 by class-balanced construction). Plan B Phase 4 training (3 seeds, warm-start from A2.5): UAR 0.6624 ± 0.0031 (Δ +0.006 vs A2.5, ~1.6σ); recall pattern more cold-balanced (recC +0.04, recNC -0.02); MLP probe ~unchanged; LR probe slightly UP +0.003. cos(init_A2.5, final) = 0.9999/1.0000/0.9997 — optimizer didn't move from A2.5's converged state; +0.006 UAR is MLP/classifier-level regularization, NOT layer-weight refinement. **3-D gate verdict: PARTIAL PASS** (gate 1 UAR floor PASS, gates 2a/2b speaker probe drop FAIL, gate 3 mix-bit PASS by construction). α=0.775 mean (22.5% partner contribution) too gentle to activate de-confounding mechanism. **α ∈ [0.5, 0.7] sweep queued (~30 min, plan §4.8.8)** — three branches: (a) UAR holds + probe drops → strict PASS at aggressive α; (b) UAR holds + probe stays → confirmed limitation, lock conservative-α as canonical "modest UAR / null on probe", move to A6; (c) UAR drops + probe drops → Pareto trade-off. See (M9) for symmetric failure-mode framing ("audio splicing too detectable to use" vs "embedding mixup at safe α too undetectable to teach") and (A4) for the rung's locked-pending-sweep status.
