...
- **A2's WavLM features (multinomial LR probe, `honesty/probe.py`)** → top-1 = 0.0674 (~14× chance) — what every A5a/A5b/A5d audit measures. LR with L2 extracts more speaker information from a 4096-d substrate than the MLP probe recovers under its training schedule, so the LR number is **the apples-to-apples ceiling** for any audit comparison.
- **A3 manner-pooled features (deep MLP probe)** → top-1 = 0.0555 — *more* leakage than A2 under the same probe, hence rejected.
- A speaker probe trained on the *same* features as the cold classifier tells you how much of the cold model's prediction could plausibly be speaker shortcut. Within an audit family, always compare numbers from the *same* probe substrate.
